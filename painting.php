...
<head>
    ...
    <link rel="stylesheet" href="./css/painting.css?v=<?php echo time();?>">
        ...
        <div class="full-width" id="my_photo_work">
            <div class="fixed-width clearfix">
                <div class="col-xl-12 col-12">
                    <h1 class="t-center title">造型設計作品</h1>
                </div>
                <div class="col-xl-12 col-12">
                <?php
                    $stmt = $mysql->query("SELECT pic_type,category,work.id w_id,work_path,title
                    FROM work WHERE `category` = 'painting' AND `title` = '造型設計'");
                    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    foreach($rows as $row){ 
                        if($row["pic_type"] == 1){  //如果為直式
                        ?>
                ...
                <div class="col-xl-12 col-12">
                    <h1 class="t-center title">影像處理作品</h1>
                </div>
                <div class="col-xl-12 col-12">
                <?php
                    $stmt = $mysql->query("SELECT pic_type,category,work.id w_id,work_path,title
                    FROM work WHERE `category` = 'painting' AND `title` = '影像處理'");
                    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
                    foreach($rows as $row){ 
                        if($row["pic_type"] == 1){
                        ?>
            ...
            if($(this).attr("class") == "click_me" || $(this).attr("class") == "my_comment"){
                ...
                $.get(
                    ...
                    for(var i=0;i<comment.length;i++){
                        ...
                        console.log(comment[i].work_path);
                        ...
                        if(comment[i].user_id != null){
                            ...
                            res = res + "<p class='no_comment t-center'><strong>尚無留言!</strong></p>";
                        }
                    }
