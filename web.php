...
<body>
    ...
    <header class="full-width" id="web">
        <h1>網頁作品</h1>
    </header>
    ...
    <div class="full-width clearfix my_work">
        <div class="fixed-width">
            <div class="col-xl-12 col-12">
                <h1 class="t-center">網頁設計作品</h1>
            </div>

            <div class="col-xl-12 col-12">
                <?php
                    $stmt = $mysql->query("SELECT pic_type,category,work.id w_id,work_path
                    FROM work WHERE `category` = 'web_page'");
                    ...
                    <div class="work_bg">
                        <div class="img">
                           <!-- <div class="web_content"> -->
                                <iframe src="<?php echo $row["work_path"]; ?>" frameborder="0"></iframe>
                           <!-- </div>  -->
                           <div class="on_web">
                                <iframe src="<?php echo $row["work_path"]; ?>" frameborder="0"></iframe>
            ...
            <?php } ?>
            <div class="col-xl-12 col-12 remind">
                <h1 class="t-center">將會陸續加入其他作品~~</h1>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function(){
            ...
            if($(this).attr("class") == "click_me" || $(this).attr("class") == "my_comment"){
                ...
                $.get(
                    ...
                    for(var i=0;i<comment.length;i++){
                        $(".wrapper iframe").attr("src",comment[i].work_path);
        ...
        $(document).on("click",".edit_msg_sent",function(){
                ...
                $(this).siblings("input").replaceWith("<li class='comment'>" + value + "</li>");
                $(this).fadeOut();
        });
    ...
    //     axios.get("./php/getuser.php?id=<?php //echo $_SESSION['id']; ?>")
    //        .then(function (response){
    //            for(i=0;i<response.data.length;i++){
    //                vm.user = response.data[i].name;
    //                 console.log(response.data[i].image);
    //                if(response.data[i].image != null){
    //                    vm.image = "../user_image/" + response.data[i].image;
    //                }else{
    //                    vm.image = "https://ecourse.ncyu.edu.tw/theme/image.php/eguru/core/1617860656/u/f1";
    //                }
    //             }
    //        })
